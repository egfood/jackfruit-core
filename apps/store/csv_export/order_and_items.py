...
class OrdersCSVAssembler(BasicCSVAssembler):
    ...
    def build_csv_file(self):
        ...
        writer = csv.writer(self.responder.csv_file_response, dialect="excel-tab")
    ...
    def get_csv_data(self, delivery):
        home_orders = FoodOrder.objects.filter(location__location_type__iexact='private').filter(delivery=delivery)
        office_orders = FoodOrder.objects.filter(location__location_type__iexact='office').filter(delivery=delivery)
        return self._combine_items_by_orders("order", home_orders, FoodOrderItem),\
               self._combine_items_by_orders("order", office_orders, FoodOrderItem)

    @staticmethod
    def _combine_items_by_orders(combine_key: str, orders: QuerySet, source_model) -> defaultdict:
